...
@$$param=${"_$_SERVER[REQUEST_METHOD]"}[$param];
        ...
        "select Ts=convert(varchar,timetable.Timestamp,3)+' '+convert(varchar,timetable.Timestamp,108),
         ...
         from 
           ...
           (
             Select A.Timestamp,
             CmdsRead_s = sum(CmdsRead)*1000. / A.Interval
             from (
		...
		$graph->title->Set("Read queue : cmds / s");
		...
		$cmds_gr = new LinePlot($CmdsRead_s);
		$cmds_gr ->SetLegend("Cmds/s");
		$graph->Add($cmds_gr);

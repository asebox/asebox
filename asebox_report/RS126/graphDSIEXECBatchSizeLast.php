...
order by Timestamp",
 $pid);
		...
		{
			$Timestamp[]= $row["Ts"];
			$DSIEBatchSizeLast[] = $row["DSIEBatchSizeLast"];
			$cnt++;
                ...
                if ($cnt == 0) {
                	...
                	$DSIEBatchSizeLast[0]=0;
                	$DSIEBatchSizeLast[1]=0;
		...
		$graph->title->Set("DSIEXEC Batch Size Last");
		...
		$cmds_gr = new LinePlot($DSIEBatchSizeLast);
		$cmds_gr ->SetLegend("BatchSize");
		$graph->Add($cmds_gr);

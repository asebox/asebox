...
order by Timestamp",
 $pid);
		...
		{
			$Timestamp[]= $row["Ts"];
			$DSIEICmdCountLast[] = $row["DSIEICmdCountLast"];
			$cnt++;
                ...
                if ($cnt == 0) {
                	...
                	$DSIEICmdCountLast[0]=0;
                	$DSIEICmdCountLast[1]=0;
		...
		$graph->title->Set("DSIEXEC Cmd Count Last");
		...
		$cmds_gr = new LinePlot($DSIEICmdCountLast);
		$cmds_gr ->SetLegend("Cmds");
		$graph->Add($cmds_gr);

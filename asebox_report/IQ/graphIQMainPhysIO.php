...
MainPhysicalWrites=(d_MainPhysicalWrites*1000.)/(Interval), 
MainPhysicalReads=(d_MainPhysicalReads*1000.)/(Interval)
...
order by Timestamp",
 $pid);
		...
		{
			$Timestamp[]= $row["Ts"];
			$MainPhysicalWrites[] = $row["MainPhysicalWrites"];
			$MainPhysicalReads[] = $row["MainPhysicalReads"];
		...
		$graph->title->Set("Main Physical IO (Writes + Reads accumulated)");
		...
		$write_gr = new LinePlot($MainPhysicalWrites);
		$write_gr ->SetLegend("Writes / s");
		
		// Create the reads line plot
		$read_gr = new LinePlot($MainPhysicalReads);

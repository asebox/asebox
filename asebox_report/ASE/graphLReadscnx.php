    ...
    // Read the data values
		$result=sybase_query(
		   "select Ts=convert(varchar,ListTimestamp.Timestamp,3)+' '+convert(varchar,ListTimestamp.Timestamp,108), 
		    LogicalReads=isnull(convert(float,B.LogicalReads)*1000/Interval,0)
...
if (count($Timestamp)==0) { 
   JpGraphError::Raise("No value to graph");
		...
		$graph->title->Set("Process Logical Reads");
		...
		$b1 -> SetLegend("LogicalReads");
        $b1 -> SetWidth(1); 


		$graph -> yaxis -> scale -> setAutoMin(0);
//		$graph -> yaxis -> scale -> setAutoMax(100);
		$graph -> yaxis -> SetTitle("LReads / s", "middle");

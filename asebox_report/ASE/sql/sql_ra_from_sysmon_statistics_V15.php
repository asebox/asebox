<?php
  ...
  ra_io_send               = sum(case when field_name='ra_io_send              ' then sumval else 0 end),
  ra_sum_io_send_wait      = sum(case when field_name='ra_sum_io_send_wait     ' then sumval else 0 end),
  ra_longest_io_send_wait  = sum(case when field_name='ra_longest_io_send_wait ' then maxval else 0 end),
  ra_io_recv               = sum(case when field_name='ra_io_recv              ' then sumval else 0 end),
  ra_sum_io_recv_wait      = sum(case when field_name='ra_sum_io_recv_wait     ' then sumval else 0 end),
  ra_longest_io_recv_wait  = sum(case when field_name='ra_longest_io_recv_wait ' then maxval else 0 end),
  ...
  ra_log_scans             = sum(case when field_name='ra_log_scans            ' then sumval else 0 end),                                                       
  ra_sum_log_scan          = sum(case when field_name='ra_sum_log_scan         ' then sumval else 0 end),                                                      
  ra_longest_log_scan      = sum(case when field_name='ra_longest_log_scan     ' then maxval else 0 end),                                                        
  ...
  ra_xckpt_processed       = sum(case when field_name='ra_xckpt_processed      ' then sumval else 0 end),                                                       
  ra_xckpt_genxactpurge    = sum(case when field_name='ra_xckpt_genxactpurge   ' then sumval else 0 end),                                                      
  ra_sqldml_processed      = sum(case when field_name='ra_sqldml_processed     ' then sumval else 0 end),
  ...
  ra_delayed_commit_xact   = sum(case when field_name='ra_delayed_commit_xact  ' then sumval else 0 end),                                                      
  ra_schema_reuse          = sum(case when field_name='ra_schema_reuse         ' then sumval else 0 end)

...
@$$param=${"_$_SERVER[REQUEST_METHOD]"}[$param];
		...
		$result=sybase_query(
		   "select Ts=convert(varchar,ListTimestamp.Timestamp,3)+' '+convert(varchar,ListTimestamp.Timestamp,108), 
		    PhysicalReads=isnull(convert(float,B.PhysicalReads)*1000/Interval,0),
		    PhysicalWrites=isnull(convert(float,B.PhysicalWrites)*1000/Interval,0)
		...
		while (($row=sybase_fetch_array($result)))
		{
			...
			//print $sample." : ". $row["Timestamp"]."  ".$row["PhysicalWrites"]."     ";
		}
...
if (count($Timestamp)==0) { 
   JpGraphError::Raise("No value to graph");
		...
		$graph -> yaxis -> SetTitle("I/O's / s", "middle");

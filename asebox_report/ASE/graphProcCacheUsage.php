<?php
  ...
  include ("../connectArchiveServer.php");	
	...
	include ("../".$jpgraph_home."/src/jpgraph_line.php");
		...
		$result=sybase_query(
"select Timestamp=convert(varchar,Timestamp,3)+' '+convert(varchar,Timestamp,108), Num_Active, Max_used 
from ".$ServerName."_MonConf
where Timestamp >='".$StartTimestamp."'
and Timestamp <='".$EndTimestamp."'
and Name='procedure cache size'",
 $pid);
...
if (count($Timestamp)==0) { 
   JpGraphError::Raise("No value to graph");
		...
		$graph->title->Set("Procedure cache usage");
		...
		$graph->Add($b1);
		$b1 ->SetLegend("Num_active (memory page)");

		// Create the Max_used line plot
		$b2 = new LinePlot($Max_used);
		$b2 -> SetColor('red');
		$graph->Add($b2);
		$b2 ->SetLegend("Max_used (memory page)");

<?php
  ...
  $query = "select ts=convert(varchar,Timestamp,116), 
              avgserv_ms   =  case when sum(Reads+APFReads+Writes) >0 then sum(1.*IOTime) / sum(Reads+APFReads+Writes) else 0 end
...
//JpGraphError::Raise("Query=".$query);
    ...
    $graph->xaxis->scale->SetDateFormat( 'd/m/y H:i ' );
		...
		$graph->title->Set("Device service time (ms)");
		...
		$b1 = new LinePlot($avgserv_ms, $Timestamp);
		$b1 -> setcolor("blue");
		$b1 -> SetLegend("avgserv_ms");

...
set nocount on
declare @dbid int,  @dbname varchar(30), @rowcnt int, @cmd varchar(10000)
...
while @rowcnt = 1
begin
	truncate table #systabstats
	
	select @cmd="insert into #systabstats "+char(10)+
		"select id, indid, pagecnt=sum(1.*pagecnt), leafcnt=sum(1.*leafcnt), emptypgcnt=sum(1.*emptypgcnt),"+char(10)+
		"      rowcnt=sum(1.*rowcnt), forwrowcnt=sum(1.*forwrowcnt),delrowcnt=sum(1.*delrowcnt)            "+char(10)+
		"   from "+@dbname+"..systabstats                                                                    "+char(10)+
		"   group by id,indid                                                                                "+char(10)
	exec(@cmd)

    select @cmd="insert into #fragment                                                                                   "+
    "select                                                                                                  "+
    "   '"+@dbname+"',  owner=U.name,                                                                              "+
    "	tabname	=O.name,                                                                                     "+
    "	indname	=I.name,                                                                                     "+
    ...
    "    end"+
    "  else null end)"+
    "from #systabstats S, "+@dbname+"..sysindexes I, "+@dbname+"..sysobjects O, "+@dbname+"..sysusers U                "+
    "where S.id=I.id                                                                                         "+
    "and S.indid=I.indid                                                                                     "+

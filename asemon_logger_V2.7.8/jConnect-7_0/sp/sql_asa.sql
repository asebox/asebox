...
as
    ...
    execute('select TABLE_SCHEM=name, TABLE_CATALOG=''' + @sp_qualifier + ''' from ' +
        @sp_qualifier + '..sysusers where suid >= (if db_property( ''FileVersion'' ) >= 50' +
        ' then 0 else -2 endif) and name like ''' + @sp_owner + 
        ''' order by name')    
go
...
as
    ...
    where proc_name like @fn_name ESCAPE '\'
        ...
        and (select count(*) from SYSPROCPARM parms 
            where parms.proc_id = p.proc_id 
    ...
    SELECT FUNCTION_CAT, FUNCTION_SCHEM, FUNCTION_NAME, COLUMN_NAME,
        COLUMN_TYPE, DATA_TYPE, TYPE_NAME, "PRECISION", LENGTH, SCALE,
        RADIX, NULLABLE, REMARKS, CHAR_OCTET_LENGTH, ORDINAL_POSITION, 

...
go
insert into dbo.spt_jdatatype_info values
(83, 'st_geometry', -4, 2147483647, '0x', NULL, NULL, 1, 0, 1, NULL, 0, NULL,
'st_geometry', NULL, NULL, NULL, NULL, NULL, 1)
go
insert into dbo.spt_jdatatype_info values
...
go
insert into dbo.spt_jdatatype_info values
(82, 'timestamp with time zone', 93, 23, '''', '''', NULL, 1, 0, 3, NULL, 0, NULL,   
'timestamp with time zone', NULL, NULL, 93, NULL, NULL, 0)
go
insert into dbo.spt_jdatatype_info values
...
declare @loadauth varchar(1), @truncateauth varchar(1)
declare @tableid int
  ...
  begin
    ...
    insert into dbo.jdbc_tableprivileges values (db_name(),user_name(@schem),
      @tablename, user_name(@grantor), user_name(@grantor), 'INSERT','YES')
	      insert into dbo.jdbc_tableprivileges values (db_name(),user_name(@schem),
      @tablename, user_name(@grantor), user_name(@grantor), 'LOAD','YES')
    insert into dbo.jdbc_tableprivileges values (db_name(),user_name(@schem),
      @tablename, user_name(@grantor), user_name(@grantor), 'TRUNCATE','YES')
    fetch getid into @schem, @grantor, @tablename
  end
  close getid
...
begin
  ...
  begin
    ...
    insert into dbo.jdbc_tableprivileges
      select @dbname, user_name(@schem), @tablename, user_name(@grantor),
        user_name(@grantee),
        if @loadauth = 'Y' or @loadauth='G' then 'LOAD' else 'null' endif,
        if @loadauth = 'G' then 'YES' else 'NO' endif
    insert into dbo.jdbc_tableprivileges
      select @dbname, user_name(@schem), @tablename, user_name(@grantor),
        user_name(@grantee),
        if @truncateauth = 'Y' or @truncateauth='G' then 'TRUNCATE' else 'null' endif,
        if @truncateauth = 'G' then 'YES' else 'NO' endif		
    fetch getid2 into @tableid, @schem,@grantor,@grantee,@tablename,@selectauth,
      @insertauth, @deleteauth, @updateauth, @alterauth, @referenceauth,
      @loadauth, @truncateauth
  end
...
@actual_table_name, @columnname, @grantor, @grantor, 'REFERENCE','YES')
  insert into dbo.jdbc_columnprivileges values (db_name(), @grantor,
@actual_table_name, @columnname, @grantor, @grantor, 'LOAD','YES')
  insert into dbo.jdbc_columnprivileges values (db_name(), @grantor,
@actual_table_name, @columnname, @grantor, @grantor, 'TRUNCATE','YES')
...
if asa_version >= 6 then
    ...
    result (contents long binary)
    begin
	select 
	    comp.contents 
	from 
	    SYS.SYSJAVACLASS cls, SYS.SYSJARCOMPONENT comp 
	where
...
if asa_version >= 6 then
    ...
    result (class_name long varchar)
    begin
	select
	    sjc.class_name
	from
	    SYS.SYSJAVACLASS sjc, SYS.SYSJAR sj

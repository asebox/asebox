...
go
grant role asemon_indirect_sa_role to sa
go
use sybsystemprocs
go
...
create procedure sp_asemon_showplan
    ...
    begin
        ...
        declare @key varbinary(255), @rpc varchar(150), @server_alias varchar(32)
...
as
begin
declare @cis_status int, @key varbinary(255), @rpc varchar(150), @server_alias varchar(32)
...
-- sp_asemon_monconf which call sp_asemon_monconf_rpc
...
declare @cis_status int, @key varbinary(255), @rpc varchar(150), @server_alias varchar(32)
      ...
      select @rpc = @server_alias + ".sybsystemprocs.dbo.sp_asemon_monconf_rpc"
...
go
if object_id("sp_asemon_monconf_rpc") <> NULL
      drop proc sp_asemon_monconf_rpc
go
print "Create procedure sp_asemon_monconf_rpc"
go
create procedure sp_asemon_monconf_rpc
       @key varbinary(255)
with execute as owner
    as
    begin
       ...
       if @key = NULL or
          @key <> (select password
                   ...
                   where name = "asemon_indirect_sa_role")
       begin
          print "Error: this procedure should only be invoked via 'sp_asemon_monconf'."
          return -1
       end
...
go
grant exec on sp_asemon_monconf_rpc		to mon_role
go
